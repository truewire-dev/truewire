...
1. **Record, one endpoint at a time.** For each endpoint the inventory marked `record:
   ...
   shows>" --new base_url=<url> [--new api_key=$KEY] [--scrub <secret field>]`. The
   request is API-named parameters as JSON; the command calls through your client, writes
   `examples/<id>.request.json` and `.response.json`, and runs `check` on the pair.
   - A paginated endpoint gets a real walk: `page1`, `page2`, ... until the terminator the
     `pagination` block declares actually fires (a short page, an empty page, an absent
     cursor). Pin the walk to a fixed starting point where the API allows it (`sha`,
     `since`, an id), so re-recording yields the same pages.
   ...
   - WebSocket streams are recorded by hand today: subscribe with the core's socket
     ...
     stream's `examples/`; `examples/kraken` shows every file shape.
2. **Generate.** `truewire generate python`. The generator runs pyright when the project
   has a `pyrightconfig.json` (copy `examples/github/pyrightconfig.json`); zero errors.
3. **Tests.** Create `test/conftest.py` with a `client` fixture that builds the real root
   client against `truewire.mock.running_mock_servers(project_root)`, and
   `test/test_examples_replay.py` assigning `truewire.testing.build_http_replay_test(root)`
   (and `build_ws_replay_test` for streams). Add one test per `_paged` walk asserting the
   page shape the recordings show. `PYTHONPATH=src pytest -q` must pass; a 422 from the
   mock means the client sent something no recording matches, and a 409 means two
   recordings match one request: collapse them.
4. **Gates.** `truewire examples --require-verified` (every endpoint has a pair or a
   reason), `truewire surface` (every spec produces a callable), `truewire check`.
...
- Do not commit a recording that contains a credential; `--scrub` it.
