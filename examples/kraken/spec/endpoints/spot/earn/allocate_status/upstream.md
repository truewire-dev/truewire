...
Returns the status of the most recently dispatched allocation request for a strategy.
Requires either the `Earn Funds` or `Query Funds` API key permission. Only one
(de)allocation can be in flight per user/strategy pair, so this always resolves to
...
[Get Allocation Status](https://docs.kraken.com/api-reference/earn/get-allocation-status)
