...
class Schema(BaseModel):
    ...
    not_: ReferenceOr["Schema"] | None = Field(default=None, alias="not")
    ...
    def handle_nullable(self) -> "Schema":
        ...
        if isinstance(self.type, str):
            ...
            if 'null' not in self.type:
                self.type.append('null')
        elif len(self.oneOf) > 0:
            self.oneOf.append(Schema(type='null'))
        elif len(self.anyOf) > 0:
            self.anyOf.append(Schema(type='null'))
        elif len(self.allOf) > 0:  # Nullable allOf is basically oneOf[null, allOf]
            self.oneOf = [Schema(type='null'), Schema(allOf=self.allOf)]
            self.allOf = []
        return self
